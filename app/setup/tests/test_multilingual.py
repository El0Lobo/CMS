...
class SeedDataTests(TestCase):
    """Test that seed data creates properly translated content."""

    def setUp(self):
        self.user = User.objects.create_user(username="admin", password="admin123")

    def test_seed_data_creates_multilingual_pages(self):
        """Test that seeded pages have all language variants."""
        from django.core.management import call_command

        # Run seed command
        call_command("seed_db")

        # Check home page has all translations
        home = Page.objects.filter(navigation_order=0).first()
        self.assertIsNotNone(home)

        self.assertEqual(home.title_en, "Home")
        self.assertEqual(home.title_es, "Inicio")
        self.assertEqual(home.title_de, "Startseite")
        self.assertEqual(home.title_fr, "Accueil")

    def test_login_page_same_slug_all_languages(self):
        """Test that seeded login page has /login slug in all languages."""
        from django.core.management import call_command

        call_command("seed_db")

        # Find login page (navigation_order=99)
        login_page = Page.objects.filter(navigation_order=99).first()
        self.assertIsNotNone(login_page)

        # All slugs should be "login"
        self.assertEqual(login_page.slug_en, "login")
        self.assertEqual(login_page.slug_es, "login")
        self.assertEqual(login_page.slug_de, "login")
        self.assertEqual(login_page.slug_fr, "login")
