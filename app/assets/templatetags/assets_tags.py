...
def render_asset(context, asset_ref, **kwargs):
    ...
    if ref.isdigit():
        ...
        asset = Asset.objects.filter(collection__slug=col_slug, slug=a_slug).select_related("collection").first()
    else:
    ...
    html = render_to_string("assets/partials/block_item.html", {"asset": asset, "mode": mode, "request": context.get("request")})
...
def render_collection(context, collection_slug, **kwargs):
    ...
    assets = col.assets.all().select_related("collection").prefetch_related("tags").order_by("title")
    if limit: assets = assets[:limit]

    html = render_to_string("assets/partials/block_collection.html", {
        "collection": col,
        "assets": assets,
        "layout": layout,
        "mode": mode,
