...
from app.comms.models import MessageThread, Message, ReadReceipt, UserThreadState, Attachment
...
def detail(request, thread_id:int):
    ...
    if getattr(request.user, 'is_superuser', False) and not getattr(request, 'impersonating', False):
        base_qs = base_qs.filter(Q(audiences__user=request.user) | Q(messages__sender_user=request.user)).distinct()
    thread = get_object_or_404(base_qs.prefetch_related("messages__attachments"), pk=thread_id)

    # Mark read
    for m in thread.messages.all():
        ReadReceipt.objects.get_or_create(message=m, user=request.user, defaults={"read_at": timezone.now()})
